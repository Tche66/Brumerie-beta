...
export function VerifiedTag({ tier, isVerified, isPremium, size = 'md', showBoth = false }: VerifiedTagProps) {
  ...
  const effectiveVerified = isVerified || tier === 'verified' || tier === 'premium';
  const effectivePremium = isPremium || tier === 'premium';

  if (showBoth) {
    const badges = [];
    if (effectiveVerified) badges.push(
      <span key="v" className={`inline-flex items-center gap-1 font-semibold rounded-full ${pad[size]} ${txt[size]}`}
    ...
    if (effectivePremium) badges.push(
      <span key="p" className={`inline-flex items-center gap-1 font-semibold rounded-full ${pad[size]} ${txt[size]}`}
