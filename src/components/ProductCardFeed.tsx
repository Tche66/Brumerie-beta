...
// Carte verticale pleine largeur pour le feed social — style Instagram/Depop
...
export function ProductCardFeed({
  ...
  const imgSrc = (product.images?.length ? product.images[0] : null) || (product as any).imageUrl || null;
  const [saved, setSaved] = useState(isBookmarked);
  const [isLiked, setIsLiked] = useState(false);
  const [likeCount, setLikeCount] = useState(0);
  const [likeLoading, setLikeLoading] = useState(false);
  const [currentImg, setCurrentImg] = useState(0);
  const images = product.images?.length ? product.images : [imgSrc].filter(Boolean) as string[];

  useEffect(() => { setSaved(isBookmarked); }, [isBookmarked]);

  // Compteur likes temps réel
  useEffect(() => {
  ...
  const pct = crossedPrice ? Math.round(((crossedPrice - displayPrice) / crossedPrice) * 100) : 0;
  ...
  return (
    <div className="bg-white rounded-[2rem] overflow-hidden border border-gray-100 shadow-sm">
      ...
      {/* ── Image principale ── */}
      <div className="relative aspect-square bg-gray-50 overflow-hidden cursor-pointer" onClick={onClick}>
        <img
          ...
          <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-1.5">
            {images.slice(0, 5).map((_, idx) => (
              <button key={idx} onClick={e => { e.stopPropagation(); setCurrentImg(idx); }}
                className={`w-1.5 h-1.5 rounded-full transition-all ${idx === currentImg ? 'bg-white w-3' : 'bg-white/60'}`}
              />
            ))}
          </div>
        )}
      ...
      <div className="flex items-center gap-4 px-4 pt-3 pb-1">
        ...
        >
          ...
          {likeCount > 0 && <span className="text-[12px] font-black text-slate-700">{likeCount.toLocaleString('fr-FR')}</span>}
        </button>

        {/* Commentaire */}
        <button onClick={onClick} className="flex items-center gap-1.5 active:scale-90 transition-transform">
          <svg width="21" height="21" viewBox="0 0 24 24" fill="none" stroke="#1a1a18" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/>
          </svg>
          {(product as any).commentCount > 0 && (
            <span className="text-[12px] font-black text-slate-700">{(product as any).commentCount}</span>
          )}
        </button>
