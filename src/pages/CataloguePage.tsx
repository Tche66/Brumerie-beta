  ...
  if (!userProfile?.isPremium) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex flex-col items-center justify-center px-6 pb-24">
        ...
        <h2 className="font-semibold text-[20px] text-slate-900 text-center mb-2">🖼️ Catalogue WhatsApp</h2>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 pb-24 font-sans">
      ...
      <div className="bg-white sticky top-0 z-50 px-4 py-4 flex items-center gap-3 border-b border-slate-100 shadow-sm">
        ...
        <div className="flex-1">
          <h1 className="font-semibold text-[14px] text-slate-900">🖼️ Catalogue WhatsApp</h1>
          <p className="text-xs font-medium text-slate-400">{choisis.length}/{actifs.length} articles sélectionnés</p>
        </div>
      </div>
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        {loading ? (
          ...
          <div className="text-center py-14 bg-white rounded-xl border-2 border-dashed border-slate-100">
            <p className="text-4xl mb-3">🛍️</p>
            <p className="font-semibold text-slate-400 text-sm mb-1">Aucun article en ligne</p>
            <p className="text-xs font-medium text-slate-300">Publie des articles pour créer ton catalogue</p>
          </div>
        ) : (
          <>
            {/* Type de message */}
            <div className="bg-white rounded-lg p-4 border border-slate-100 shadow-sm">
              <p className="text-xs font-medium text-slate-400 mb-3">Type de message</p>
            ...
            <div className="bg-white rounded-lg border border-slate-100 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-slate-50">
                <p className="text-xs font-medium text-slate-400">Choisir les articles</p>
                <button onClick={toggleAll}
                  className="text-xs font-medium text-green-600 active:scale-95">
            ...
            {choisis.length > 0 && (
              <div className="bg-white rounded-lg p-4 border border-slate-100 shadow-sm">
                <p className="text-xs font-medium text-slate-400 mb-3">Aperçu du message</p>
                <div className="bg-slate-50 rounded-lg p-4 max-h-52 overflow-y-auto">
            ...
            {choisis.length > 0 && (
              <div className="space-y-2">
                ...
                <button onClick={async () => {
                  ...
                  className="w-full py-4 rounded-lg bg-emerald-600 font-medium text-sm text-white active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                ...
                <a href={whatsappUrl} target="_blank" rel="noopener noreferrer"
                  className="w-full py-4 rounded-lg bg-[#25D366] font-medium text-sm text-white active:scale-[0.98] transition-all flex items-center justify-center gap-2 block text-center">
                ...
                <button onClick={handleCopy}
                  className={`w-full py-3.5 rounded-lg font-medium text-sm active:scale-[0.98] transition-all ${
