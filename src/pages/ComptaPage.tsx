...
// Fix critique : tous les hooks AVANT tout return conditionnel
...
interface Entry {
  ...
  date: string; source: 'brumerie' | 'manuel'; orderId?: string; createdAt?: any;
}

function todayISO() { return new Date().toISOString().slice(0, 10); }
function formatFCFA(n: number) { return n.toLocaleString('fr-CI') + ' FCFA'; }
...
function KpiCard({ icon, label, value, color, sub }: {
  ...
  return (
    <div className="flex-1 bg-white rounded-2xl p-4 border border-slate-100 shadow-sm min-w-0">
      ...
      <p className="font-black text-[15px] leading-none" style={{ color }}>
        {value >= 0 ? '+' : ''}{formatFCFA(value)}
      </p>
      {sub && <p className="text-[9px] text-slate-400 mt-1 leading-tight">{sub}</p>}
    </div>
  );
}
...
export function ComptaPage({ onBack }: ComptaPageProps) {
  ...
  const [form, setForm]         = useState({ type: 'recette' as EntryType, label: '', amount: '', date: todayISO() });
  const [saving, setSaving]     = useState(false);
  const [saved, setSaved]       = useState(false);

  const isVerified = !!(userProfile?.isVerified || (userProfile as any)?.isPremium);

  // Charger entrées — seulement si vérifié
  useEffect(() => {
    if (!currentUser || !isVerified) { setLoading(false); return; }
  ...
  // Injecter ventes Brumerie automatiquement — seulement si vérifié
  useEffect(() => {
    if (!currentUser || !isVerified) return;
    const unsub = subscribeOrdersAsSeller(currentUser.uid, async (orders) => {
      ...
      for (const order of delivered) {
        if (!existingIds.has(order.id)) {
          ...
          } catch { /* réessayé au prochain render */ }
        }
      }
    });
    return unsub;
  }, [currentUser?.uid, isVerified, entries.length]);

  // ── RETURN CONDITIONNEL après tous les hooks ──
  if (!isVerified) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-8 text-center pb-20">
        <div className="w-20 h-20 bg-slate-100 rounded-3xl flex items-center justify-center text-4xl mx-auto mb-5">🔒</div>
        <h2 className="font-black text-slate-900 text-xl uppercase tracking-tight mb-2">Vendeur Vérifié requis</h2>
        ...
        <button onClick={onBack}
          className="px-6 py-3.5 bg-green-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest active:scale-95 transition-all shadow-lg shadow-green-200">
          Retour
        </button>
      </div>
    );
  ...
  const viaBrumerie   = filtered.filter(e => e.type === 'recette' && e.source === 'brumerie').reduce((s, e) => s + e.amount, 0);
  ...
  const handleAdd = async () => {
    ...
    try {
      await addDoc(collection(db, 'compta'), {
        ...
        date: form.date, source: 'manuel', createdAt: serverTimestamp(),
      });
  ...
  const handleExport = () => {
    const rows = [['Date', 'Type', 'Libellé', 'Montant (FCFA)', 'Source'],
      ...filtered.map(e => [e.date, e.type, e.label, String(e.amount), e.source])];
    const csv = rows.map(r => r.join(';')).join('\n');
    const blob = new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `brumerie-compta-${todayISO()}.csv`; a.click();
    ...
    <div className="min-h-screen bg-slate-50 pb-24 font-sans">
      ...
      <div className="bg-white sticky top-0 z-50 px-4 py-4 flex items-center gap-3 border-b border-slate-100 shadow-sm">
        ...
        <button onClick={handleExport}
          className="flex items-center gap-1.5 bg-slate-100 px-3 py-2 rounded-xl text-[9px] font-black text-slate-600 uppercase tracking-widest active:scale-95">
        ...
        <div className="space-y-2">
          <div className="flex gap-2">
            <KpiCard icon="📈" label="Recettes" value={recettes} color="#16A34A"
              sub={`dont ${formatFCFA(viaBrumerie)} via Brumerie`}/>
          ...
          <div className="space-y-3">
            {loading ? (
              ...
              <div className="text-center py-12 bg-white rounded-3xl border-2 border-dashed border-slate-100">
                ...
                <p className="text-[10px] text-slate-300 mb-4">Tes ventes Brumerie s'ajoutent automatiquement</p>
                <button onClick={() => setTab('ajouter')}
                  className="px-5 py-2.5 bg-green-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest active:scale-95">
                  + Saisir une entrée
                </button>
              </div>
            ) : (
                      ...
                      <div>
                        ...
                        <div className="h-2.5 bg-red-100 rounded-full overflow-hidden">
                          <div className="h-full bg-red-400 rounded-full"
                            style={{ width: `${Math.min((depenses / Math.max(recettes, 1)) * 100, 100)}%` }}/>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="text-[11px] font-bold text-slate-700 flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-green-500 inline-block"/>Via Brumerie
                      </span>
                      <span className="font-black text-[13px] text-green-700">{formatFCFA(viaBrumerie)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[11px] font-bold text-slate-700 flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-blue-400 inline-block"/>Hors plateforme
                      </span>
                      <span className="font-black text-[13px] text-blue-700">{formatFCFA(recettes - viaBrumerie)}</span>
                    </div>
                  </div>
                </div>
                {benefice < 0 && (
                  <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-amber-700 leading-snug">Tes dépenses dépassent tes recettes. Vérifie tes coûts de stock et de transport.</p>
                    </div>
                  </div>
                )}
                {recettes > 0 && viaBrumerie < recettes * 0.5 && (
                  <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 flex gap-3">
                    <span className="text-2xl flex-shrink-0">🚀</span>
                    <div>
                      <p className="font-black text-blue-800 text-[12px] mb-1">Astuce</p>
                      <p className="text-[11px] text-blue-700 leading-snug">Passe plus de ventes par Brumerie pour avoir un historique complet automatique.</p>
                    </div>
                  </div>
                )}
              </>
              ...
              {([
                ...
                <button key={t.id} onClick={() => setForm(f => ({ ...f, type: t.id }))}
                  className={`py-3 rounded-2xl border-2 font-black text-[11px] uppercase tracking-wide transition-all ${
                    form.type === t.id
                      ? t.color === 'green' ? 'bg-green-50 border-green-500 text-green-700' : 'bg-red-50 border-red-400 text-red-600'
                    ...
                    {dayEntries.map(e => (
                      <div key={e.id} className="bg-white rounded-2xl px-4 py-3 border border-slate-100 shadow-sm flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <span className={`text-[8px] font-black px-2 py-0.5 rounded-full mt-0.5 inline-block ${
