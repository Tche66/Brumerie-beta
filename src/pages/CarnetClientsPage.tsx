  ...
  if (!userProfile?.isVerified && !userProfile?.isPremium) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex flex-col items-center justify-center px-6 pb-24">
        ...
        <h2 className="font-semibold text-[20px] text-slate-900 text-center mb-2">📇 Carnet Clients</h2>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 pb-24 font-sans">
      ...
      <div className="bg-white sticky top-0 z-50 px-4 py-4 flex items-center gap-3 border-b border-slate-100 shadow-sm">
        ...
        <div className="flex-1">
          <h1 className="font-semibold text-[14px] text-slate-900">📇 Carnet Clients</h1>
          <p className="text-xs font-medium text-slate-400">{totalClients} clients · {fmt(totalCA)} CA total</p>
        </div>
        {view === 'liste' && (
          <button onClick={() => setView('ajouter')}
            className="flex items-center gap-1.5 bg-emerald-600 text-white px-3 py-2 rounded-xl font-medium text-sm active:scale-[0.98] shadow-sm">
        ...
        <div className="px-4 pt-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-white rounded-xl p-3 text-center border border-slate-100 shadow-sm">
              <p className="text-xs font-medium text-slate-400 mb-1">Clients</p>
              <p className="font-semibold text-[17px] text-slate-900">{totalClients}</p>
            </div>
            <div className="bg-white rounded-xl p-3 text-center border border-slate-100 shadow-sm">
              <p className="text-xs font-medium text-slate-400 mb-1">CA Total</p>
              <p className="font-semibold text-[11px] text-green-700 leading-tight">{fmt(totalCA)}</p>
            </div>
            <div className={`rounded-xl p-3 text-center border ${aRelancer.length > 0 ? 'bg-amber-50 border-amber-200' : 'bg-slate-50 border-slate-100'}`}>
              <p className="text-xs font-medium text-slate-400 mb-1">À relancer</p>
              <p className={`font-semibold text-[17px] ${aRelancer.length > 0 ? 'text-amber-600' : 'text-slate-400'}`}>{aRelancer.length}</p>
            </div>
          </div>
          ...
          {aRelancer.length > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="font-semibold text-amber-800 text-[12px]">{aRelancer.length} client{aRelancer.length > 1 ? 's' : ''} sans achat depuis 30j+</p>
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Rechercher..." className="w-full pl-9 pr-4 py-2.5 rounded-lg border border-slate-100 bg-white text-[12px] outline-none focus:border-emerald-500"/>
            </div>
            <select value={sortBy} onChange={e => setSortBy(e.target.value as any)}
              className="px-3 py-2.5 rounded-lg border border-slate-100 bg-white text-[10px] font-bold text-slate-600 outline-none">
          ...
          {loading ? (
            ...
            <div className="text-center py-14 bg-white rounded-xl border border-dashed border-slate-100">
              ...
              <p className="font-semibold text-slate-400 text-[12px] mb-1">Aucun client</p>
            ...
            <div className="space-y-2">
              {displayed.map(cl => (
                <button key={cl.id} onClick={() => { setSelected(cl); setView('detail'); }}
                  className="w-full bg-white rounded-xl p-4 border border-slate-100 active:scale-[0.98] transition-all shadow-sm text-left flex items-center gap-3">
                  <div className={`w-11 h-11 rounded-lg flex items-center justify-center font-semibold text-base flex-shrink-0 ${
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <p className="font-semibold text-slate-900 text-[13px] truncate">{cl.nom}</p>
                      {cl.tags?.includes('VIP') && <span className="text-xs font-medium bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full flex-shrink-0">⭐ VIP</span>}
                    </div>
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="font-semibold text-[12px] text-green-700">{fmt(cl.totalAchats || 0)}</p>
      ...
      {view === 'ajouter' && (
        <div className="px-4 pt-4">
          <div className="bg-white rounded-xl p-5 border border-slate-100 shadow-sm space-y-4">
            <p className="font-semibold text-slate-900 text-[15px]">Nouveau client</p>
            {[
            ...
            <div>
              <p className="text-xs font-medium text-slate-500 mb-1.5">ID Brumerie (optionnel)</p>
              <input value={(form as any).brumarieUid}
                ...
                className="w-full px-4 py-3.5 rounded-lg border border-slate-100 bg-slate-50 text-[12px] outline-none focus:border-emerald-500 transition-all"/>
            ...
            <button onClick={handleAdd} disabled={saving || !form.nom.trim()}
              className="w-full py-4 rounded-lg font-medium text-sm text-white bg-emerald-600 active:scale-[0.98] disabled:opacity-40 transition-all">
        ...
        <div className="px-4 pt-4 space-y-3">
          ...
          <div className="bg-white rounded-xl p-5 border border-slate-100 shadow-sm">
            <div className="flex items-start gap-4 mb-4">
              <div className={`w-14 h-14 rounded-lg flex items-center justify-center font-semibold text-2xl flex-shrink-0 ${
                (selected.totalAchats || 0) > 50000 ? 'bg-amber-100 text-amber-600' : 'bg-green-100 text-green-700'
              }`}>
                {selected.nom.charAt(0).toUpperCase()}
              </div>
              <div className="flex-1">
                <h2 className="font-semibold text-slate-900 text-xl leading-tight">{selected.nom}</h2>
                {selected.phone && (
            ...
            {selected.tags?.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-4">
                {selected.tags.map(t => (
                  <span key={t} className="font-medium text-xs bg-green-100 text-green-700 px-2.5 py-1 rounded-full">{t}</span>
                ))}
              </div>
            )}
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
              <div className="bg-slate-50 rounded-lg p-3 text-center">
                <p className="text-xs font-medium text-slate-400 mb-0.5">CA Total</p>
                <p className="font-semibold text-[12px] text-green-700">{fmt(selected.totalAchats || 0)}</p>
              </div>
              <div className="bg-slate-50 rounded-lg p-3 text-center">
                <p className="text-xs font-medium text-slate-400 mb-0.5">Achats</p>
                <p className="font-semibold text-[14px] text-slate-800">{selected.nbCommandes || 0}</p>
              </div>
              <div className="bg-slate-50 rounded-lg p-3 text-center">
                <p className="text-xs font-medium text-slate-400 mb-0.5">Dernier</p>
                <p className="font-semibold text-[10px] text-slate-700 leading-tight">{daysAgo(selected.dernierAchat) || '—'}</p>
              </div>
            </div>
          ...
          <div className="space-y-2">
            <button onClick={() => setAchatModal(selected)}
              className="w-full py-4 rounded-lg font-medium text-sm text-white bg-emerald-600 active:scale-[0.98] transition-all">
              🛍 Enregistrer un achat
            </button>

            {/* Contact — Brumerie en primaire, WhatsApp en secondaire */}
            <p className="text-xs font-medium text-slate-400 px-1">Contacter</p>
            ...
            {onOpenChat && (
              <button
                ...
                className="w-full py-3.5 rounded-lg font-medium text-sm text-white bg-emerald-600 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-60">
                {lookupLoading
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <a key={m.type} href={whatsappMsg(selected, m.type as any)} target="_blank" rel="noopener noreferrer"
                  className="py-3 rounded-lg text-white font-medium text-xs text-center active:scale-95 transition-all block"
            ...
            <button onClick={() => setDeleteConfirm(selected.id)}
              className="w-full py-3.5 rounded-lg border border-red-100 text-red-500 font-medium text-sm active:scale-[0.98] bg-white">
      ...
      {achatModal && (
        <div className="fixed inset-0 z-[200] flex items-end justify-center p-4"
          ...
          <div className="bg-white rounded-xl w-full max-w-sm p-6 space-y-4"
            ...
            <p className="font-semibold text-slate-900 text-[15px]">🛍 Enregistrer un achat</p>
            ...
            <input type="date" value={achatForm.date} max={new Date().toISOString().slice(0,10)}
              ...
              className="w-full px-4 py-3.5 rounded-lg border border-slate-100 bg-slate-50 text-[13px] outline-none focus:border-emerald-500"/>
            <div className="flex gap-3">
              <button onClick={() => setAchatModal(null)}
                className="flex-1 py-3.5 rounded-lg bg-slate-100 text-slate-600 font-medium text-sm active:scale-[0.98]">
                Annuler
              </button>
        ...
        <div className="fixed inset-0 z-[200] flex items-end justify-center p-4"
          ...
          <div className="bg-white rounded-xl w-full max-w-sm p-6 space-y-4"
            ...
            <div className="text-center">
              ...
              <p className="font-semibold text-slate-900 text-[15px] mb-1">Supprimer ce client ?</p>
              <p className="text-[12px] text-slate-400">Tout son historique sera perdu.</p>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setDeleteConfirm(null)}
                className="flex-1 py-3.5 rounded-lg bg-slate-100 text-slate-600 font-medium text-sm active:scale-[0.98]">Annuler</button>
              <button onClick={handleDelete}
                className="flex-[2] py-3.5 rounded-lg bg-red-500 text-white font-medium text-sm active:scale-[0.98] shadow-sm">Supprimer</button>
            </div>
          </div>
        </div>
      )}
