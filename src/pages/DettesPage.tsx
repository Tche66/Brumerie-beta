  ...
  if (!userProfile?.isPremium) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex flex-col items-center justify-center px-6 pb-24">
        ...
        <h2 className="font-semibold text-[20px] text-slate-900 text-center mb-2">Journal de Dettes</h2>
        ...
        <button onClick={onBack}
          className="w-full max-w-xs py-4 rounded-xl bg-slate-100 text-slate-600 font-medium text-sm active:scale-95 transition-all">
          ← Retour
        </button>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 pb-24 font-sans">
      ...
      <div className="bg-white sticky top-0 z-50 px-4 py-4 flex items-center gap-3 border-b border-slate-100 shadow-sm">
        ...
        <div className="flex-1">
          <h1 className="font-semibold text-[14px] text-slate-900">📒 Journal de Dettes</h1>
        ...
        {view === 'liste' && (
          <button onClick={() => setView('ajouter')}
            className="flex items-center gap-1.5 bg-emerald-600 text-white px-3 py-2 rounded-xl font-medium text-sm active:scale-[0.98] transition-all shadow-sm">
        ...
        <div className="px-4 pt-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-white rounded-lg p-3 border border-slate-100 shadow-sm text-center">
              <p className="text-xs font-medium text-slate-400 mb-1">En cours</p>
              <p className="font-semibold text-[15px] text-red-600">{actives.length}</p>
            </div>
            <div className={`rounded-xl border p-3 text-center ${enRetard.length > 0 ? 'bg-red-50 border-red-200' : 'bg-slate-50 border-slate-100'}`}>
              <p className="text-xs font-medium text-slate-400 mb-1">En retard</p>
              <p className={`font-semibold text-[15px] ${enRetard.length > 0 ? 'text-red-600' : 'text-slate-400'}`}>{enRetard.length}</p>
            </div>
            <div className="bg-green-50 rounded-lg p-3 border border-green-100 shadow-sm text-center">
              <p className="text-xs font-medium text-green-600 mb-1">Total dû</p>
              <p className="font-semibold text-[11px] text-green-700 leading-tight">{fmt(totalDu)}</p>
            </div>
          </div>
          ...
          {enRetard.length > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-3 flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="font-semibold text-red-700 text-[12px]">{enRetard.length} paiement{enRetard.length > 1 ? 's' : ''} en retard</p>
          ...
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              ...
              className="w-full pl-10 pr-4 py-3 rounded-lg border border-slate-100 bg-white text-[12px] outline-none focus:border-emerald-500 transition-all"/>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-14 bg-white rounded-xl border border-dashed border-slate-100">
              <p className="text-4xl mb-3">📒</p>
              <p className="font-semibold text-slate-400 text-[12px] mb-1">
              ...
              <button onClick={() => setView('ajouter')}
                className="px-5 py-2.5 bg-emerald-600 text-white rounded-lg font-medium text-sm active:scale-[0.98]">
                + Ajouter une dette
              </button>
            </div>
          ) : (
              ...
              {filtered.map(d => {
                ...
                return (
                  <button key={d.id} onClick={() => openDetail(d)}
                    className={`w-full text-left bg-white rounded-xl p-4 border active:scale-[0.98] transition-all shadow-sm ${
                      overdue ? 'border-red-200' : d.statut === 'solde' ? 'border-green-100' : 'border-slate-100'
                    }`}>
                    <div className="flex items-start gap-3">
                      {/* Avatar initiale */}
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center font-semibold text-base flex-shrink-0 ${
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-0.5">
                          <p className="font-semibold text-slate-900 text-[13px] truncate">{d.clientNom}</p>
                          <span className={`font-semibold text-[13px] ml-2 flex-shrink-0 ${
                        ...
                        <div className="flex items-center gap-2 mt-1.5">
                          ...
                          {overdue && <span className="text-[8px] font-semibold bg-red-100 text-red-600 px-2 py-0.5 rounded-full">EN RETARD</span>}
                          {d.statut === 'partiel' && <span className="text-[8px] font-semibold bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">PARTIEL</span>}
      ...
      {view === 'ajouter' && (
        <div className="px-4 pt-4">
          <div className="bg-white rounded-xl p-5 border border-slate-100 shadow-sm space-y-4">
            <p className="font-semibold text-slate-900 text-[15px]">Nouvelle dette client</p>
            ...
            {[
              ...
              <div key={f.key}>
                <p className="text-xs font-medium text-slate-500 mb-1.5">{f.label}</p>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <p className="text-xs font-medium text-slate-500 mb-1.5">Montant total *</p>
                <div className="relative">
                  <input value={form.montant}
                    ...
                    className="w-full px-3 py-3.5 rounded-lg border border-slate-100 bg-slate-50 text-[13px] font-semibold outline-none focus:border-emerald-500 transition-all pr-14"/>
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[9px] font-bold text-slate-400">FCFA</span>
                </div>
              </div>
              <div>
                <p className="text-xs font-medium text-slate-500 mb-1.5">Déjà payé</p>
                <div className="relative">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <p className="text-xs font-medium text-slate-500 mb-1.5">Date de vente</p>
                <input type="date" value={form.date} max={todayISO()}
                  onChange={e => setForm(f => ({ ...f, date: e.target.value }))}
                  className="w-full px-3 py-3.5 rounded-lg border border-slate-100 bg-slate-50 text-[12px] outline-none focus:border-emerald-500 transition-all"/>
              </div>
              <div>
                <p className="text-xs font-medium text-slate-500 mb-1.5">Échéance (optionnel)</p>
            ...
            <div>
              <p className="text-xs font-medium text-slate-500 mb-1.5">Note (optionnel)</p>
              <textarea value={form.note}
                onChange={e => setForm(f => ({ ...f, note: e.target.value }))}
                placeholder="Ex: Paie le vendredi au marché"
                rows={2}
                className="w-full px-4 py-3 rounded-lg border border-slate-100 bg-slate-50 text-[12px] outline-none focus:border-emerald-500 transition-all resize-none"/>
            </div>

            <button onClick={handleAdd}
              disabled={saving || !form.clientNom.trim() || !form.article.trim() || !form.montant}
              className="w-full py-4 rounded-lg bg-emerald-600 font-medium text-sm text-white active:scale-[0.98] disabled:opacity-40 transition-all">
              {saving
      ...
      {view === 'detail' && selected && (() => {
        ...
        return (
          <div className="px-4 pt-4 space-y-3">
            {/* Carte principale */}
            <div className={`bg-white rounded-xl p-5 border shadow-sm ${overdue ? 'border-red-200' : selected.statut === 'solde' ? 'border-green-200' : 'border-slate-100'}`}>
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h2 className="font-semibold text-slate-900 text-xl">{selected.clientNom}</h2>
              ...
              <div className="bg-slate-50 rounded-lg p-4 mb-4 space-y-2">
                ...
                <div className="flex justify-between border-t border-slate-200 pt-2">
                  <span className="text-[12px] font-semibold text-slate-700">Reste à payer</span>
                  <span className={`text-[14px] font-semibold ${selected.statut === 'solde' ? 'text-green-600' : 'text-red-600'}`}>{fmt(restant)}</span>
                </div>
              </div>
              ...
              {selected.montant > 0 && (
                <div className="mb-4">
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-[9px] font-semibold text-green-600">{Math.round(pct)}%</span>
                  </div>
            ...
            <div className="space-y-2">
              {selected.statut !== 'solde' && (
                <>
                  ...
                  <button onClick={() => { setPayModal(selected); setPayAmount(String(restant)); }}
                    className="w-full py-4 rounded-lg bg-emerald-600 font-medium text-sm text-white active:scale-[0.98] transition-all">
                  ...
                  {onOpenChat && (
                    <button
                      ...
                      className="w-full py-4 rounded-lg bg-emerald-600 font-medium text-sm text-white active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-60">
                      {lookupLoading
                  ...
                  <a href={whatsappRappel(selected)} target="_blank" rel="noopener noreferrer"
                    className="w-full py-4 rounded-lg bg-emerald-500 font-medium text-sm text-white active:scale-[0.98] transition-all flex items-center justify-center gap-2 block text-center">
              ...
              <button onClick={() => setDeleteConfirm(selected.id)}
                className="w-full py-3.5 rounded-lg border border-red-100 text-red-500 font-medium text-sm active:scale-[0.98] bg-white transition-all">
      ...
      {payModal && (
        <div className="fixed inset-0 z-[200] flex items-end justify-center p-4"
          ...
          <div className="bg-white rounded-xl w-full max-w-sm p-6 space-y-4"
            ...
            <p className="font-semibold text-slate-900 text-[15px]">💰 Enregistrer un paiement</p>
            ...
            <div className="relative">
              <input value={payAmount}
                ...
                className="w-full px-4 py-4 rounded-lg border border-slate-100 bg-slate-50 text-[16px] font-semibold outline-none focus:border-emerald-500 transition-all pr-20"/>
              ...
              {[25, 50, 100].map(pct => {
                ...
                return (
                  <button key={pct} onClick={() => setPayAmount(String(val))}
                    className="flex-1 py-2 rounded-xl bg-slate-100 text-slate-600 font-semibold text-[10px] active:scale-95">
                    {pct}%
                  </button>
                );
              })}
              <button onClick={() => setPayAmount(String(payModal.montant - payModal.montantPaye))}
                className="flex-1 py-2 rounded-xl bg-green-100 text-green-700 font-semibold text-[10px] active:scale-95">
                Tout
              </button>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setPayModal(null)}
                className="flex-1 py-3.5 rounded-lg bg-slate-100 text-slate-600 font-medium text-sm active:scale-95">
                Annuler
              </button>
        ...
        <div className="fixed inset-0 z-[200] flex items-end justify-center p-4"
          ...
          <div className="bg-white rounded-xl w-full max-w-sm p-6 space-y-4"
            ...
            <div className="text-center">
              ...
              <p className="font-semibold text-slate-900 text-[15px] mb-1">Supprimer cette dette ?</p>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleDelete}
                className="flex-[2] py-3.5 rounded-lg bg-red-500 text-white font-medium text-sm active:scale-[0.98] shadow-sm">
                Supprimer
              </button>
            </div>
          </div>
